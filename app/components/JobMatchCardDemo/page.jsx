...
const JobMatchCard = ({ name, jobCount }) => {
  return (
    <div className="relative w-full max-w-[587px] h-auto sm:h-[460px] border border-slate-300 bg-blue-50 rounded-3xl overflow-hidden p-6">
      <Image src={img1} className="bottom-0 absolute left-0" alt="Decoration" />
      <Image src={img2} className="absolute right-0" alt="Background" />

      {/* Content */}
      <div className="relative z-10">
        <h2 className="text-xl sm:text-2xl lg:text-3xl font-bold mb-2 mt-7 px-5 text-slate-700">
          {name}, we found over{' '}
          <span className="text-blue-600">{jobCount} jobs</span> <br /> that
          match your skills
        </h2>
        <p className="text-sm sm:text-base lg:text-lg text-gray-500 mb-6 mt-1 px-5">
          Let's take a look at them, you might as well find<br /> your dream job
          ✨
        </p>
        <div className="px-5 mt-8 sm:mt-12">
